...
Превращаем полученную строку в хэш и генерируем имя. Так же добавляем
соль к хэшу, поскольку у нас могут встречаться одинаковые выражения во
время компиляции, которые, тем не менее, не должны иметь одинаковые
имена.
#+NAME: generate_name
#+BEGIN_SRC scheme :noweb yes
...
соответствующих конструкторов, определенных в файле ~primitives.c~.
...
#+BEGIN_SRC scheme :noweb yes
  ...
  (print-code (compile ''((define n 10)
                          (define f 4))
                       global-environment))

  (print-code (compile ''(n
                          (define f 4))
                       global-environment))

  (print-code (compile ''(n f)
                       global-environment))
#+END_SRC

~Сборка~

Теперь необходимо сделать так, что скомпилированный код компилировался
вместе с функцией main.

В процессе сборки я столкнулась с некоторой проблемой.
Намжелательно сделать так, чтоб юзер увидел результат исполнения
скомпилированного выражения. Поэтому нужно напечатать его, а все прочие -
вызвать.

Но вот в чем загвозка: выражение вполне может быть определением функции
или лямбды, и вот их вызывать не нужно автоматически.
Поэтому мы будем определять тип последнего выражения - ведь именно его
результат мы будем печатать - , которое ввел юзер. Если жто определение
функции или лямбды, то мы просто удалим их вызовы из списка вызовов,
которые представляют собой тело main.

Также видно, что в main всегда впомиливается вызов ~set_true_and_false~ -
это функция определена в primitives.c и нужна, чтоб установить глобальные
переменные true и false для примитивов, написанных на си. Безэтого они не
будут работать корректно.

#+NAME: final_compile_with_main
#+BEGIN_SRC scheme :noweb yes
...
Это тесты всей системы.
Сначала нужно запустить драйвер луп. Внутри него вызывается установка
окружения компилятора. Как можно видеть, это происходит после каждого
вызова драйвер-лупа, следовательно, каждое выражение будет скомпилировано
заново, с чистого листа.
Поэтому если нужно скомпилировать сразу несколько выражений, то они
объединяются в блок ~begin~.

ЧТоб правильно скомпилировать сишный код, нужно в косноли ввести
"gcc primitives.c your-file-name.c -o name"

~primitives.c~ - это файл, в котором определены все примитивы в сишном
эквиваленте.
