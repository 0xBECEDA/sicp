...
#+BEGIN_SRC scheme :noweb yes
  ...
  (define (compile-null? args)
    (append '(null_predicate)
            (compile-parenthesis args)
            (list SEMICOLON)
            ))

  (define (compile-pair? args)
    (append '(pair_predicate)
            (compile-parenthesis args)
            (list SEMICOLON)
            ))

  (define (compile-atom? args)
    (append '(atom_predicate)
            (compile-parenthesis args)
            (list SEMICOLON)
            ))
...
#+BEGIN_SRC scheme :noweb yes
  ...
  (define (connect-all-exps lst)
    (define (connect-all-exps-rec lst new-lst)
      (if (null? lst)
          new-lst
          (connect-all-exps-rec (cdr lst) (append new-lst (car lst)))))
    (connect-all-exps-rec lst '()))
  ...
  (define (compile-primitive exp env)
    (define (compile-cur-primitive compiled-args-names)
      ...
      (cond ((eq? (operator exp) 'list)
            ...
            ((eq? (operator exp) 'add)
             (compile-add compiled-args-names))
            ((eq? (operator exp) 'sub)
             (compile-sub compiled-args-names))
            ((eq? (operator exp) 'mul)
             (compile-mul compiled-args-names))
            ((eq? (operator exp) 'division)
             (compile-division compiled-args-names))
            ...
            (else (error "Неизвестный примитив -- compile-primitive"))))
    (let* ((compiled-args
           ...
           (compiled-args-names (map (lambda (code)
                                     ...
                                     compiled-args))
           (pre-result  (append
                         (connect-all-exps (map (lambda (code)
                                (format #t "code ~A ~%" code)
                                (car code))
                              compiled-args))
                        '(return)
  ...
  (print-code (compiled-code->string (compile-primitive '(list 1 2 3 4) '())))
#+END_SRC
...
#+BEGIN_SRC forth :tangle code.scheme  :noweb tangle :exports code :padline no :comments none
  ...
  <<trasform_code>>
  <<print_code>>
  <<compile_int_and_char_constructors>>
