  ...
  (define (compile-list args)
    (define (compile-list-rec args compiled-code)
      (if (null? args)
          compiled-code
          (if (null? (cdr args))
              (compile-list-rec (cdr args) (append compiled-code (list (car args))))
              (compile-list-rec (cdr args) (append compiled-code (list (car args))
                                                   (list COMMA))))))
    (let ((amount-of-args (length args)))
      (append '(make_list)
              (compile-parenthesis
               (append (compile-list-rec args (list amount-of-args COMMA))))
              (list SEMICOLON))))
...
#+BEGIN_SRC scheme :noweb yes
  ...
  (define (compile-variable exp env)
    (if (true? (lookup-variable exp env))
...
#+BEGIN_SRC scheme :noweb yes
  ...
  ;; (compile-application '(+ x 2)  (cons '(x) global-environment))
  ...
  (define (compile-primitive exp env)
    ...
    (let* ((compiled-args
           ...
           (pre-result (append (connect-compiled-exps compiled-args)
                               '(return)
                               (compile-cur-primitive compiled-args-names)))
...
#+BEGIN_SRC scheme :noweb yes
  ...
  ;;                                (append compiled-code (compile (car sequenses) env)))))
  ;;   (format #t "compile-sequences ~A ~%" compile-sequences)
  ;;   (compile-sequences-rec sequenses '()))

  (define (compile-sequences sequenses env)
    (define (compile-sequences-rec sequenses)
      (if (null? sequenses)
          '()
          (cons (compile (car sequenses) env)
                (compile-sequences-rec (cdr sequenses)))))
    ;; (format #t "sequenses ~A ~%" sequenses)
    ;; (format #t "sequenses env ~A ~%" env)
    (let* ((compiled-code-with-names (compile-sequences-rec sequenses)))
...
#+BEGIN_SRC scheme :noweb yes
  (define (compile-lambda exp env)
    (compile-function (generate-name-string exp) (lambda-parameters exp)
                      (lambda-body exp) env))
...
#+BEGIN_SRC scheme :noweb yes
  ...
  (define (compile-named-function exp env)
    ...
    (compile-function (definition-variable exp)
...
#+BEGIN_SRC scheme :noweb yes
  ...
  (define (compile-compound-proc-call exp env)
    (let* ((compiled-args-with-names (compile-args (operands exp) env))
      ...
      (append (list 'val ASTERISK service-fn-name)
              (compile-parenthesis '())
...
#+BEGIN_SRC scheme :noweb yes
  ...
  ;; (print-code (compiled-code->string (compile-if '(if (+ 1 2)
  ;;                                                     2
  ;;                                                     3) global-environment)))

  ;; (print-code (compiled-code->string (compile-if '(if (true? 1)
  ;;                                                     2
  ;;                                                     3) global-environment)))
